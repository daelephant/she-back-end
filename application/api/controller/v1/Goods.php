...
//获取当前用户上传的订单
    ...
    public function update_status(){
        ...
        $putData = input('put.');
        $sData = [];
        if (array_key_exists('is_on_sale',$putData) && $putData['is_on_sale']){
            $sData['is_on_sale']  = $putData['is_on_sale'];
        }
        if (array_key_exists('check_status',$putData)){
            $sData['check_status']  = $putData['check_status'];
        }
        $sData['updatetime'] = date('Y-m-d H:i:s',time());
        //条件
        if (array_key_exists('id',$putData) && $putData['id']){
            $search['id']  = $putData['id'];
        }
        //$search['id']  = $putData['id'];
        $search['user_id']  = $putData['user_id'];
        ...
        //通过更新状态
            ...
            $configdb2 = config('setting.db_config2');
            $good = Db::connect($configdb2)->table('p2018_goods')->where($search)->update($sData);
            if ($good){
                return json(new SuccessMessage(),200);
            }else{
                throw new GoodsException();
